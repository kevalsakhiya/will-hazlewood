...
class AgentSpider(Spider):
    ...
    def parse(self, response):
        agent_url_list = response.xpath(
            './/div[@data-testid="AgentList"]//a/@href'
        ).getall()
        print("AGENT URLS: ", agent_url_list)
    ...
    def parse_agent(self, response):
        ...
        brn = jmespath.search("compliances[0].value", agent_data)
        ...
        item["listings_for_sale"] = (
            int(listings_for_sale) if listings_for_sale else None
        )
        ...
        item["listings_for_rent"] = (
            int(listings_for_rent) if listings_for_rent else None
        )
        ...
        item["closed_transaction_sale"] = (
            int(closed_transaction_sale) if closed_transaction_sale else None
        )

        closed_transaction_rent = jmespath.search("claimedTransactionsRent", agent_data)
        item["closed_transaction_rent"] = (
            int(closed_transaction_rent) if closed_transaction_rent else None
        )

        closed_transaction_deal_volume = jmespath.search(
            "claimedTransactionsDealVolume", agent_data
        )
        item["closed_transaction_deal_value"] = (
            float(closed_transaction_deal_volume)
            if closed_transaction_deal_volume
            else None
        )

        closed_transaction_sale_avg_amount = jmespath.search(
            "claimedTransactionsSaleAVGAmount", agent_data
        )
        item["closed_transaction_sale_avg_amount"] = (
            float(closed_transaction_sale_avg_amount)
            if closed_transaction_sale_avg_amount
            else None
        )

        closed_transaction_rent_avg_amount = jmespath.search(
            "claimedTransactionsRentAVGAmount", agent_data
        )
        item["closed_transaction_rent_avg_amount"] = (
            float(closed_transaction_rent_avg_amount)
            if closed_transaction_rent_avg_amount
            else None
        )

        closed_transaction_rent_total_amount = jmespath.search(
            "claimedTransactionsRentTotalAmount", agent_data
        )
        item["closed_transaction_rent_total_amount"] = (
            float(closed_transaction_rent_total_amount)
            if closed_transaction_rent_total_amount
            else None
        )

        closed_transaction_sale_total_amount = jmespath.search(
            "claimedTransactionsSaleTotalAmount", agent_data
        )
        item["closed_transaction_sale_total_amount"] = (
            float(closed_transaction_sale_total_amount)
            if closed_transaction_sale_total_amount
            else None
        )

        transaction_records_list = jmespath.search(
            "claimedTransactionsList", agent_data
        )
        transaction_records_list = (
            transaction_records_list if transaction_records_list else []
        )
        for transaction in transaction_records_list:
            ...
            if "Rent" in deal_type:
                if deal_date and (
                    most_recent_deal_date_rent is None
                    or deal_date > most_recent_deal_date_rent
                ):
                ...
                if deal_date and (
                    most_recent_deal_date_sale is None
                    or deal_date > most_recent_deal_date_sale
                ):
                    most_recent_deal_date_sale = deal_date
                total_monthly_deal_volume_sale += 1

        average_monthly_deal_volume_rent = total_monthly_deal_volume_rent / 12
        average_monthly_deal_volume_sale = total_monthly_deal_volume_sale / 12

        item["closed_deals_total"] = (item.get("closed_transaction_sale") or 0) + (
            item.get("closed_transaction_rent") or 0
        ) or None
        ...
        item["most_recent_deal_date_rent"] = (
            most_recent_deal_date_rent.strftime("%Y-%m-%d")
            if most_recent_deal_date_rent
            else None
        )
        item["most_recent_deal_date_sale"] = (
            most_recent_deal_date_sale.strftime("%Y-%m-%d")
            if most_recent_deal_date_sale
            else None
        )
    ...
    def parse_property(self, response):
        ...
        listings_with_marketing_spend = property_data.get(
            "listings_with_marketing_spend", 0
        )
        ...
        total_listing_age_days_rent = property_data.get(
            "total_listing_age_days_rent", 0
        )
        total_listing_age_days_sale = property_data.get(
            "total_listing_age_days_sale", 0
        )
        most_recent_listing_date_rent = property_data.get(
            "most_recent_listing_date_rent", None
        )
        most_recent_listing_date_sale = property_data.get(
        ...
        for property in property_list:
            ...
            is_spotlight_listing = jmespath.search(
                "property.is_spotlight_listing", property
            )
            ...
            listed_date = (
                datetime.strptime(listed_date, "%Y-%m-%dT%H:%M:%SZ")
                if listed_date
                else None
            )
            ...
            if property_type == "Residential for Rent":
                ...
                property_rent_price = (
                    int(property_rent_price) if property_rent_price else 0
                )
                ...
                most_recent_listing_date_rent = (
                    ...
                    if listed_date
                    and (
                        most_recent_listing_date_rent is None
                        or listed_date > most_recent_listing_date_rent
                    )
                    else most_recent_listing_date_rent
                )

            elif property_type == "Residential for Sale":
                property_sale_price = jmespath.search("property.price.value", property)
                property_sale_price = (
                    int(property_sale_price) if property_sale_price else 0
                )
                ...
                most_recent_listing_date_sale = (
                    ...
                    if listed_date
                    and (
                        most_recent_listing_date_sale is None
                        or listed_date > most_recent_listing_date_sale
                    )
                    else most_recent_listing_date_sale
                )
        ...
        item["most_recent_listing_date_rent"] = (
            most_recent_listing_date_rent.strftime("%Y-%m-%d")
            if most_recent_listing_date_rent
            else None
        )
        item["most_recent_listing_date_sale"] = (
            most_recent_listing_date_sale.strftime("%Y-%m-%d")
            if most_recent_listing_date_sale
            else None
        )

        item["average_listing_price_rent"] = (
            total_property_rent_price / item["listings_for_rent"]
            if item.get("listings_for_rent")
            else None
        )
        item["average_listing_price_sale"] = (
            total_property_sale_price / item["listings_for_sale"]
            if item.get("listings_for_sale")
            else None
        )
        item["average_listing_age_days_rent"] = (
            total_listing_age_days_rent / item["listings_for_rent"]
            if item.get("listings_for_rent")
            else None
        )
        item["average_listing_age_days_sale"] = (
            total_listing_age_days_sale / item["listings_for_sale"]
            if item.get("listings_for_sale")
            else None
        )
